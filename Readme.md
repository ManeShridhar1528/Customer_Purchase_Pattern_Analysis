
Customer Purchase Pattern Analysis

End-to-End Data Analytics Project | Python • SQL • Power BI


Project Overview:

This project presents an end-to-end data analytics workflow to analyze customer purchase behavior and uncover actionable business insights.

The goal is to help businesses understand revenue drivers, customer segments, and growth opportunities using structured data analysis, SQL querying, and interactive dashboards.


Business Objective:

Identify key factors driving revenue

Analyze customer purchase patterns across categories

Understand customer behavior by age, gender, subscription, and shipping type

Support data-driven decisions for sales, marketing, and customer retention


Tools & Technologies Used:

Python (Pandas) – Data cleaning, preprocessing, and feature engineering

PostgreSQL – SQL analysis and business-driven queries

Power BI – Interactive dashboards and visual storytelling

GitHub – Version control and project documentation


Project Folder Structure:
Customer_Purchase_Pattern_Analysis/
│
├── 1_Raw_Data/
│   └── customer_purchase_data.csv
│
├── 2_Python_Data_Cleaning/
│   ├── data_cleaning.ipynb
│   └── cleaned_customer_data.csv
│
├── 3_SQL_Analysis/
│   ├── customer_analysis.sql
│   ├── sql_queries_and_insights.docx
│   └── sql_queries_and_insights.pdf
│
├── 4_PowerBI_Dashboard/
│   ├── Customer_Purchase_Pattern_Dashboard.pbix
│   ├── dashboard_dataset.csv
│   └── Dashboard_Screenshots/
│       ├── dashboard_1.png
│       ├── dashboard_2.png
│       ├── dashboard_3.png
│       └── dashboard_4.png
│       
│
├── 5_Project_Report/
│   └── Customer_Purchase_Pattern_Analysis_Report.pdf
│
└── README.md


Data Cleaning & Preparation (Python):

Removed duplicate records

Handled missing and inconsistent values

Standardized categorical columns

Validated data types

Prepared clean dataset for SQL and Power BI analysis


Exploratory Data Analysis (EDA):

Key analysis performed

Revenue distribution across product categories

Customer segmentation by age group and gender

Subscription vs non-subscription behavior

Impact of shipping type on purchase value

Average order value analysis


SQL Analysis (Business Questions):

SQL queries were written to answer key business questions such as:

Which product categories generate the highest revenue?

Which age group contributes most to total sales?

How does subscription status affect customer spending?

Which shipping types are linked to higher average order value?

How does revenue vary by gender and category?

Advanced SQL concepts used:

GROUP BY, CASE WHEN

Aggregate functions

Business-focused filtering and ordering


Power BI Dashboard:

An interactive Power BI dashboard was built to visualize insights clearly.

Dashboard Features:

KPI cards (Customers, Revenue, Avg Purchase, Ratings)

Revenue by product category

Subscription status distribution

Revenue by age group and gender

Average order value by shipping type

Interactive slicers (Location, Gender, Size, Season)


Key Insights:

Clothing and Accessories are the highest revenue-generating categories

Footwear shows premium buying behavior

Young Adults contribute the most to total revenue

Majority of customers are not subscribed, indicating upsell potential

Customers choosing faster shipping tend to spend more


Business Recommendations:

Increase subscription adoption through targeted offers

Focus marketing on high-performing categories

Promote premium products and bundles

Encourage faster shipping options for higher order value

Use customer segmentation for personalized campaigns


Outcome:

This project successfully transforms raw customer purchase data into clear, actionable business insights using Python, SQL, and Power BI.
It demonstrates a complete data analytics lifecycle suitable for real-world business decision-making.


Author

Shridhar Mane
Aspiring Data Analyst

